...
async fn add_group_returns_1_when_group_is_valid() -> () {
    ...
    let result = db.groups.add_role(&group_name).await;
    assert!(result.is_ok());
...
async fn get_group_by_name_returns_group_when_name_exists() -> () {
    ...
    let result = db.groups.get_role_by_name(&group_name).await;
    assert!(result.is_ok());
    let result = result.unwrap();
...
async fn get_group_by_name_returns_err_when_group_does_not_exist() -> () {
    ...
    let result = db.groups.get_role_by_name("not found").await;
    assert!(result.is_err());
}
...
async fn get_group_by_id_returns_group_when_id_is_valid() -> Result<(), sqlx::Error> {
    ...
    let result = db.groups.get_role_by_id(group.id).await;
    assert!(result.is_ok());
    let result = result.unwrap();
...
async fn update_group_returns_0_when_group_does_not_exist() -> () {
    ...
    let result = db.groups.update_role("not found", "still not found").await;
    assert!(result.is_ok());
    let result = result.unwrap();
    assert_eq!(0, result);
}
...
async fn delete_group_returns_1_when_group_can_be_deleted() -> Result<(), sqlx::Error> {
    ...
    let result = db.groups.delete_role(&group_name).await;
    assert!(result.is_ok());
    let result = result.unwrap();
    assert_eq!(1, result);
    Ok(())
...
async fn delete_group_returns_0_when_group_does_not_exist() -> () {
    ...
    let result = db.groups.delete_role("not found").await;
    assert!(result.is_ok());
    let result = result.unwrap();
    assert_eq!(0, result);
}
